...
JUMP_SIZE = 40 # Tamanho de cada tile (cada tile é um quadrado)
PLAYER_WIDTH = JUMP_SIZE
PLAYER_HEIGHT = int(JUMP_SIZE * 1.5)
...
class Bird(pygame.sprite.Sprite):
    ...
    def __init__(self, player_img, row, column, blocks):
        ...
        self.rect.x = column * JUMP_SIZE
        self.rect.bottom = row * JUMP_SIZE
...
def load_assets(img_dir):
    assets = {}
    assets[PLAYER_IMG] = pygame.image.load(path.join(img_dir, 'playerShip1_orange.png')).convert_alpha()
    assets[OBST] = pygame.image.load(path.join(img_dir, 'obstaculo1.png')).convert()
    return assets
